...
class WC_Gateway_Paynow extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Paynow', 'woo-paynow' );
		$this->method_description = __( 'Accepts payments by Paynow', 'woo-paynow' );
		$this->icon               = apply_filters( 'woo_paynow_icon', WOO_PAYNOW_PLUGIN_URL . '/assets/images/logo.png' );
		$this->supports           = [
			'products'
		];
		...
		// Get setting values.
		$this->title         = __( 'Pay by Paynow', 'woo-paynow' );
		$this->description   = __( 'Secure BLIK payments and fast online transfers', 'woo-paynow' );
	...
	private function init_paynow_client() {
		$user_agent       = 'Wordpress-' . get_bloginfo( 'version' ) . '/WooCommerce-' . WC()->version . '/Plugin-' . WOO_PAYNOW_PLUGIN_VERSION;
	...
	public function send_payment_request( $order ) {
		...
		$payment_data = [
			...
			'currency'    => $currency,
			'externalId'  => $order_id,
			'description' => __( 'Order No: ', 'woo-paynow' ) . $order_id,
			'buyer'       => [
	...
	function process_payment( $order_id ) {
		...
		try {
			...
			$order->update_status( 'on-hold', __( 'Awaiting Paynow payment', 'woo-paynow' ) );
			...
			wc_add_notice( __( 'Error occurred during the payment process and the payment could not be completed.', 'woo-paynow' ), 'error' );
	...
	public function validate_minimum_payment_amount( $order ) {
		if ( WC_Paynow_Helper::get_amount( $order->get_total() ) < WC_Paynow_Helper::get_minimum_amount() ) {
			throw new PaynowException( sprintf( __( 'Sorry, the minimum allowed order total is %1$s to use this payment method.', 'woo-paynow' ), wc_price( WC_Paynow_Helper::get_minimum_amount() / 100 ) ) );
		}
	}
}