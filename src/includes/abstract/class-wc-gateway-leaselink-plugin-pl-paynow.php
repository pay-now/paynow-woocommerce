...
abstract class WC_Gateway_Leaselink_Plugin_PL_Paynow extends WC_Payment_Gateway {
	...
	public const PAYNOW_PAYMENT_GATEWAY
		= array(
			self::BLIK_PAYMENT => WC_LEASELINK_PLUGIN_PREFIX . 'blik',
			self::PBL_PAYMENT  => WC_LEASELINK_PLUGIN_PREFIX . 'pbl',
			self::CARD_PAYMENT => WC_LEASELINK_PLUGIN_PREFIX . 'card',
		);

	/**
	 * @var Leaselink_Paynow_Gateway
	 */
	public $gateway;

	/**
	 * @var WC_Leaselink_Plugin_PL_Locking_Mechanism
	 */
	...
	public function __construct() {
		...
		$this->gateway           	= new Leaselink_Paynow_Gateway();
		$this->locking_mechanism 	= new WC_Leaselink_Plugin_PL_Locking_Mechanism();
		$this->show_payment_methods = wc_leaselink_plugin()->settings()->get_show_payment_methods();
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields = include WC_LEASELINK_PLUGIN_FILE_PATH . 'includes/settings/pay-by-paynow-pl-settings.php';
	}

	public function payment_fields() {

		include WC_LEASELINK_PLUGIN_FILE_PATH . WC_LEASELINK_PLUGIN_TEMPLATES_PATH . 'payment_processor_info.php';
	}
	...
	public function process_payment( $order_id ): array {
		...
		try {
			WC_Leaselink_Plugin_PL_Helper::validate_minimum_payment_amount( (float) $order->get_total() );
		} catch ( Exception $e ) {
			WC_Leaselink_Plugin_PL_Logger::error( $e->getMessage(), array( WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_EXTERNAL_ID_FIELD_NAME => $order_id ) );
			return $response;
		}
		...
		if ( WC_Leaselink_Plugin_PL_Helper::is_old_wc_version() ) {
			add_post_meta( $order_id, '_transaction_id', $payment_data[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME ], true );
		} else {
			$order->add_meta_data( '_transaction_id', $payment_data[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME ], true );
		}

		if ( WC_Leaselink_Plugin_PL_Helper::is_old_wc_version() ) {
			update_post_meta( $order_id, '_transaction_id', $payment_data [ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME ] );
		} else {
			$order->set_transaction_id( $payment_data[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME ] );
		}
		...
		if ( ! in_array(
			$payment_data[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_STATUS_FIELD_NAME ],
			array(
				Status::STATUS_NEW,
			...
			$response['redirect'] = $payment_data[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_REDIRECT_URL_FIELD_NAME ];
		}
	...
	protected function get_validation_errors_message( $message = '' ) {
		if ( strpos( $message, 'buyer.email' ) !== false ) {
			return __( 'Invalid email address entered. Check the correctness of the entered data', 'leaselink-plugin-pl' );
		}

		return __( 'A data validation error occurred. Check the correctness of the entered data', 'leaselink-plugin-pl' );
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		$refund_amount = WC_Leaselink_Plugin_PL_Helper::get_amount( $amount );
		...
		WC_Leaselink_Plugin_PL_Logger::debug(
		...
		try {
			...
			WC_Leaselink_Plugin_PL_Logger::info(
			...
			if ( $errors ) {
				foreach ( $errors as $error ) {
					...
					WC_Leaselink_Plugin_PL_Logger::error(
	...
	public function check_can_make_refund( $order, int $amount ): bool {
		...
		$order_id = WC_Leaselink_Plugin_PL_Helper::get_order_id( $order );

		if ( empty( $order_id ) ) {
			WC_Leaselink_Plugin_PL_Logger::warning( 'Order was not found to make a refund {orderId={}}', array( $order_id ) );

			return false;
		}

		if ( empty( $order->get_transaction_id() ) ) {
			WC_Leaselink_Plugin_PL_Logger::warning( 'The order has no payment to make a refund {orderId={}}', array( $order_id ) );

			return false;
		}

		if ( empty( $amount ) ) {
			WC_Leaselink_Plugin_PL_Logger::warning( 'The amount of the refund must be above zero {orderId={}}', array( $order_id ) );

			return false;
		}

		if ( ! $order->has_status( wc_get_is_paid_statuses() ) ) {
			WC_Leaselink_Plugin_PL_Logger::warning(
	...
	public function is_available(): bool {
		...
		if ( ! is_admin() ) {
			$available = true;
			try {
				WC_Leaselink_Plugin_PL_Helper::validate_minimum_payment_amount( WC_Leaselink_Plugin_PL_Helper::get_payment_amount() );
			} catch ( PaynowException $exception ) {
				$available = false;
			}
	 ...
	 * @param $force
	 * @return void
	 * @throws WC_Leaselink_Plugin_Pl_Notification_Retry_Processing_Exception
	 * @throws WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception
	 */
	public function process_notification( $payment_id, $status, $external_id, $modified_at = '', $force = false ) {
		...
		$context = array(
			WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_EXTERNAL_ID_FIELD_NAME => $external_id,
			WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME  => $payment_id,
			WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_STATUS_FIELD_NAME      => $status,
			WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_MODIFIED_AT_FIELD_NAME => $modified_at,
		);
		...
		if ( $this->locking_mechanism->check_and_create( $external_id ) ) {
			for ( $i = 1; $i <= 3; $i++ ) {
				...
				if ( false === $is_notification_locked ) {
					...
					throw new WC_Leaselink_Plugin_Pl_Notification_Retry_Processing_Exception( 'Skipped processing. Previous notification is still processing.', $context );
				}
			}
		}

		WC_Leaselink_Plugin_PL_Logger::info( 'Lock passed successfully, notification validation starting.', $context );
		...
		if ( ! $order ) {
			...
			throw new WC_Leaselink_Plugin_Pl_Notification_Retry_Processing_Exception(
				'Skipped processing. Order not found.',
				$context
			);
		}

		if ( strpos( $order->get_payment_method(), WC_LEASELINK_PLUGIN_PREFIX ) === false ) {
			$this->locking_mechanism->delete( $external_id );
			throw new WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception( 'Other payment gateway is already selected', $context );
		}
		...
		if ( $order_processed ) {
			...
			$this->locking_mechanism->delete( $external_id );
			throw new WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception( 'Skipped processing. Order has paid status.', $context );
		}

		if ( $order_payment_status === $status && $order_payment_id === $payment_id ) {
			$this->locking_mechanism->delete( $external_id );
			throw new WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception( sprintf( 'Skipped processing. Transition status (%s) already consumed.', $status ), $context );
		}
		...
		if ( ! empty( $order_payment_status_date ) && $order_payment_status_date > $modified_at && ! $is_confirmed ) {
			if ( ! $is_new || $order_payment_id === $payment_id ) {
				$this->locking_mechanism->delete( $external_id );
				throw new WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception(
		...
		WC_Leaselink_Plugin_PL_Logger::info( 'Order status transition is correct.', $context );
		...
		$order->save();
		$this->locking_mechanism->delete( $external_id );
		WC_Leaselink_Plugin_PL_Logger::info( 'Notification processed successfully', $context );
	}
	...
	private function retry_processing_n_times( WC_Order $order, $message, $context = array(), $counter = 3 ) {
		...
		$history_key = sprintf( '%s:%s', $context[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME ], $context[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_STATUS_FIELD_NAME ] );
		...
		$this->locking_mechanism->delete( $context[ WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_EXTERNAL_ID_FIELD_NAME ] );
		if ( $history[ $history_key ] >= $counter ) {
			throw new WC_Leaselink_Plugin_Pl_Notification_Stop_Processing_Exception( $message, $context );
		} else {
			throw new WC_Leaselink_Plugin_Pl_Notification_Retry_Processing_Exception( $message, $context );
		}
	}
	...
	public function redirect_order_received_page() {
		...
		if ( WC_Leaselink_Plugin_PL_Helper::is_paynow_order( $order ) ) {
			$payment_id = $order->get_transaction_id();
			...
			if ( $status ) {
				WC_Leaselink_Plugin_PL_Logger::info( 'Received payment status from API. ', $logger_context );
				try {
					$this->process_notification( $payment_id, $status, $order_id, gmdate( 'Y-m-d\TH:i:s' ), true );
				} catch ( Error | Exception $e ) {
					WC_Leaselink_Plugin_PL_Logger::error( $e->getMessage(), $logger_context );
				}
			}
	...
	public function allow_payment_without_login( $allcaps, $caps, $args ) {
		...
		if ( $order_key === $order_key_check && WC_Leaselink_Plugin_PL_Helper::is_paynow_order( $order ) ) {
	...
	private function process_new_status( WC_Order $order, string $payment_id, $context ) {
		...
		if ( ! empty( $order->get_transaction_id() ) && ! ( $order->get_transaction_id() === $payment_id ) ) {
			WC_Leaselink_Plugin_PL_Logger::info( 'The order has already a payment. Attaching new payment.', $context );
		}
	...
	private function get_api_option_key_name(): string {
		...
		return $this->plugin_id . WC_LEASELINK_PLUGIN_PREFIX . 'settings';
	}
	...
	protected function hooks() {
		...
		add_filter( 'woocommerce_payment_gateways', 'wc_leaselink_payment_gateways' );
	}
