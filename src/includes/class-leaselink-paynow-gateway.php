...
/**
 * Class Leaselink_Paynow_Gateway
 */
class Leaselink_Paynow_Gateway {
	...
	public function __construct() {
        ...
        $this->settings_manager = wc_leaselink_plugin()->settings();
		if ( !empty($this->settings_manager->get_api_key()) && !empty($this->settings_manager->get_signature_key()) ) {
			...
			if ( $api_key && $this->signature_key ) {
				$this->client = new Client(
					...
					'Wordpress-' . get_bloginfo( 'version' ) . '/WooCommerce-' . WC()->version . '/Plugin-' . wc_leaselink_plugin_version()
				);
			}
		}
	}
	...
	public function payment_request( WC_Order $order, $return_url, $payment_method_id = null, $authorization_code = null, $payment_method_token = null, $payment_method_fingerprint = null ) {
		...
		$currency     = WC_Leaselink_Plugin_PL_Helper::is_old_wc_version() ? $order->get_order_currency() : $order->get_currency();
		...
		$payment_data = array(
			'amount'      => WC_Leaselink_Plugin_PL_Helper::get_amount( $order->get_total() ),
		...
		if ( ! empty( $customer_id ) ) {
			$payment_data['buyer']['externalId'] = WC_Leaselink_Plugin_PL_Keys_Generator::generate_buyer_external_id( $customer_id, $this->signature_key );
		}

		$logger_context = array(
			WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_EXTERNAL_ID_FIELD_NAME => $order_id,
		);
		...
		$idempotency_key = WC_Leaselink_Plugin_PL_Keys_Generator::generate_idempotency_key( $order_id );
		...
		try {
			...
			$payment_data = array(
				WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_STATUS_FIELD_NAME       => $api_response_object->getStatus(),
				WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME   => $api_response_object->getPaymentId(),
				WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_REDIRECT_URL_FIELD_NAME => $redirect_url,
			);

			$cache_key = 'paynow_payment_methods__' . md5( substr( $this->get_signature_key(), 0, 8 ) . '_' . $currency . '_' . WC_Leaselink_Plugin_PL_Helper::get_amount( $order->get_total() ) );
			delete_transient( $cache_key );

			WC_Leaselink_Plugin_PL_Logger::debug( 'Retrieved authorization response', array_merge( $logger_context, $payment_data ) );
			...
			if ( $is_blik && ( $exception->getCode() === 504 || ! ( strpos( $exception->getMessage(), 'cURL error 28' ) === false ) ) ) {
				$payment_data = array(
					WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_STATUS_FIELD_NAME       => PaymentStatus::STATUS_NEW,
					WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_PAYMENT_ID_FIELD_NAME   => $order_id . '_UNKNOWN',
					WC_Leaselink_Plugin_PL_Helper::NOTIFICATION_REDIRECT_URL_FIELD_NAME => $return_url
				...
				WC_Leaselink_Plugin_PL_Logger::error( 'Authorization response timeout', array_merge( $logger_context, $payment_data ) );
				return $payment_data;
			} else {
				WC_Leaselink_Plugin_PL_Logger::error(
					'Authorization failed',
	...
	public function send_shop_urls_configuration_request( string $return_url ) {
		...
		try {
			...
			$shop_configuration->changeUrls( $return_url, WC_Leaselink_Plugin_PL_Helper::get_notification_url() );
		} catch ( PaynowException $exception ) {
	...
	public function payment_methods(): ?array {
		...
		$amount = WC_Leaselink_Plugin_PL_Helper::get_amount( WC_Leaselink_Plugin_PL_Helper::get_payment_amount() );
		...
		try {
			...
			if ( false === $payment_methods ) {
				...
				$buyer_external_id = $current_user_id > 0 ? WC_Leaselink_Plugin_PL_Keys_Generator::generate_buyer_external_id( $current_user_id, $this->signature_key ) : null;
	...
	public function remove_saved_instrument( $token ): bool {
		try {
			...
			$buyer_external_id = WC_Leaselink_Plugin_PL_Keys_Generator::generate_buyer_external_id( get_current_user_id(), $this->signature_key );
	...
	public function gdpr_notices( string $idempotency_key ): ?array {
		...
		try {
			...
			if ( ! is_null( WC()->session ) && ! empty( WC()->session->get( $cache_key ) ) ) {
				...
				WC_Leaselink_Plugin_PL_Logger::info( 'Retrieving GDPR notices' );
